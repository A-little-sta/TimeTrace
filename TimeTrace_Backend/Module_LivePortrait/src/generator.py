...
class Generator(object):
    def __init__(
        ...
        # Load weight
        # =========== 修改开始 (增强版) ===========
        if args.pretrained_model_path.endswith('.safetensors'):
            state_dict = load_file(args.pretrained_model_path)
        else:
            print(f"检测到 .pth 模型，切换加载方式: {args.pretrained_model_path}")
            state_dict = torch.load(args.pretrained_model_path, map_location='cpu')
            
            # 🔍 核心修复：如果发现模型被包裹在 'model_state_dict' 里，就把它取出来
            if "model_state_dict" in state_dict:
                print(">>> 发现嵌套结构，正在提取 model_state_dict 权重...")
                state_dict = state_dict["model_state_dict"]
        
        # 加载权重 (加入 strict=False 以忽略无关紧要的 buffer 缺失)
