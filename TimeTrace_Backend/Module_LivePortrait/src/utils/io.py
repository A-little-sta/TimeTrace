...
def load_image_rgb(image_path: str):
    ...
    # ✅ 修改后的代码 (支持 Windows 中文路径):
    # 先用 numpy 读取二进制流，再用 opencv 解码
    try:
    ...
    # 检查是否读取成功
    if img is None:
        raise FileNotFoundError(f"OpenCV 无法读取图片 (可能路径错误或文件损坏): {image_path}")
...
def load_img_online(obj, mode="bgr", **kwargs):
    ...
    if isinstance(obj, str):
        if mode.lower() == "gray":
            img = cv2.imdecode(np.fromfile(obj, dtype=np.uint8), cv2.IMREAD_GRAYSCALE)
        else:
            img = cv2.imdecode(np.fromfile(obj, dtype=np.uint8), cv2.IMREAD_COLOR)
    else:
        img = obj
