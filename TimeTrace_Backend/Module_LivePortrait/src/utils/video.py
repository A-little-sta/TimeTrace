...
def images2video(images, wfp, **kwargs):
    ...
    if not images:
        print("Warning: No images to write to video.")
        return

    # 获取图像尺寸
    h, w = images[0].shape[:2]
    
    # 尝试使用高性能的 H.264 编码 (avc1)
    # 如果失败，回退到通用的 mp4v
    ...
    if not writer.isOpened():
        raise RuntimeError(f"OpenCV failed to open video writer for {wfp}")

    n = len(images)
    for i in track(range(n), description='Writing', transient=True):
        img = images[i]
        
        # OpenCV 需要 BGR 格式，如果输入是 RGB 则需转换
        if image_mode.lower() == 'rgb':
            img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
            
        writer.write(img)

    writer.release()
    # print(f'Video saved to {wfp}')
