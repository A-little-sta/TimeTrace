...
class BaseInpaintingTrainingModule(ptl.LightningModule):
    def __init__(self, config, use_ddp, *args,  predict_only=False, visualize_each_iters=100,
        ...
        self.generator = make_generator(config, **self.config.training_model.generator)
        ...
        if not predict_only:
            ...
            # 只有在不是预测模式时才初始化损失函数
            if not getattr(self, 'predict_only', False):
                if self.config.training_model.losses.get("l1", {"weight_known": 0})['weight_known'] > 0:
                    self.loss_l1 = nn.L1Loss(reduction='none')

                if self.config.training_model.losses.get("mse", {"weight": 0})['weight'] > 0:
                    self.loss_mse = nn.MSELoss(reduction='none')
                
                if self.config.training_model.losses.perceptual.weight > 0:
                    self.loss_pl = PerceptualLoss()

            # ResNetPL has been removed, use PerceptualLoss instead
    ...
    def configure_optimizers(self):
        # 如果是预测模式，不配置优化器
        if getattr(self, 'predict_only', False):
            return []
    ...
    def train_dataloader(self):
        # 如果是预测模式，不初始化训练数据加载器
        if getattr(self, 'predict_only', False):
            raise NotImplementedError("预测模式下不需要训练数据加载器")
    ...
    def val_dataloader(self):
        # 如果是预测模式，不初始化验证数据加载器
        if getattr(self, 'predict_only', False):
            raise NotImplementedError("预测模式下不需要验证数据加载器")
