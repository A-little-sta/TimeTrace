...
class FourierUnit(nn.Module):
    ...
    def forward(self, x):
        ...
        # [修改] 兼容 PyTorch 1.7.1
        if hasattr(torch.fft, 'rfftn'):
            ffted = torch.fft.rfftn(x, dim=fft_dim, norm=self.fft_norm)
            ffted = torch.stack((ffted.real, ffted.imag), dim=-1)
        else:
            # PyTorch 1.7 写法
            signal_ndim = 3 if self.ffc3d else 2
            ffted = torch.rfft(x, signal_ndim=signal_ndim, normalized=(self.fft_norm == 'ortho'), onesided=True)
        ...
        # [修改] 兼容 PyTorch 1.7.1
        if hasattr(torch.fft, 'irfftn'):
            ffted = torch.complex(ffted[..., 0], ffted[..., 1])
            output = torch.fft.irfftn(ffted, s=ifft_shape_slice, dim=fft_dim, norm=self.fft_norm)
        else:
            # PyTorch 1.7 写法
            # 不需要转换为复数，直接使用实数和虚数分离的张量
            signal_ndim = 3 if self.ffc3d else 2
            output = torch.irfft(ffted, signal_ndim=signal_ndim, normalized=(self.fft_norm == 'ortho'), onesided=True, signal_sizes=ifft_shape_slice)
